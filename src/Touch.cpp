...
static volatile bool bKick_Again = false;
...
bool Check_Available_Brain_Event (void) {
...
int threadADCRead(volatile struct pt* pos)
{
  PT_BEGIN(pos);
...
int threadDisplay(volatile struct pt* disp)
{
  PT_BEGIN(disp);
...
int threadKick(volatile struct pt* sol) // Worker Thread
{
  PT_BEGIN(sol);
