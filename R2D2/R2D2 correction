...
void setup() {
  ...
  motorSetup();          // Motor Driver — sets up motor pins (Soccer_Strategy.ino)
  // groveDLSsetup();    // Light Sensor -
  // touchSetup();       // Touch Sensor -
  // eyelidSetup();      // Servo / Iris - Eyelid.ino
  PT_INIT(&pt_strategy); // Initialize the soccer strategy protothread
}

void loop() {
  //PT_SCHEDULE(eyelidThread(&ptEyelid));
  //PT_SCHEDULE(huskyRead(&ptHuskylens));
  PT_SCHEDULE(ball_distance(&pt_ball_distance));     // detect the ball with HuskyLens
  PT_SCHEDULE(goal_distance(&pt_goal_distance));     // detect the goal with HuskyLens
  PT_SCHEDULE(lens_adjustment(&pt_lens_adjustment, goal_lux)); // adjust light iris
  PT_SCHEDULE(soccer_strategy(&pt_strategy));        // run the soccer strategy brain
}
